...
Six autoloads, registered in this order (EventBus first — everything depends on it):
`EventBus`, `GameManager`, `ScoreManager`, `WaveSpawner`, `VFXManager`, `HUDManager`
— all in `framework/autoload/`. The EventBus signal catalog in `event_bus.gd` is the
system contract; extend it there, don't invent side channels.
...
2. Wave spawner & data-driven enemies — next (`WaveSpawner` is a stub; Enemy base class TBD)
3. VFX & particle pooling (`VFXManager.burst()` API is final, body is a stub)
4. Scoring & HUD (`ScoreManager` logic works; HUD scenes are stubs)
